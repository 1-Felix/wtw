...
/**
 * POST /api/services/sonarr/test — test Sonarr connection via /api/v3/system/status.
 */
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      if (!response.ok) {
        return NextResponse.json(
          { error: `Sonarr returned ${response.status}` },
          { status: 502 }
        );
      }
      ...
      return NextResponse.json(
        { error: "Could not reach Sonarr server" },
        { status: 502 }
      );
    }
  } catch (err) {
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to test Sonarr connection" },
      { status: 500 }
    );
  }
}
